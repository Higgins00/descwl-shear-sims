...
def rescale_ia_exp(*, survey, exp):
    """
    Rescale ia images noise and weight to our zero point
    ...
    None
    """
    fac = get_wldeblend_rescale_fac(survey)#need to check this
    vfac = fac**2
...
def get_ia_rescale_fac(survey):
    """
    Get the factor to rescale ia images to our zero point
    ...
    number by which to rescale images
    """
    s_zp = survey.zero_point #need to check
    s_et = survey.exposure_time #need to check
    return 10.0**(0.4*(ZERO_POINT - 24.0))/s_zp/s_et


class IASurvey(object):
    """
    wrapper for ia surveys
